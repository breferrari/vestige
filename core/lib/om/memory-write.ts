 ...
 *
 * This is not hypothetical and not misuse. It has happened at least three times
 * from the vigia repo, each recorded in the commit that carried it: "filed by
 * hand after record_work refused nine times", "filed by hand after the capture
 * step went quiet again". The session had a lesson worth keeping, the tool would
 * not take it, and it wrote a correct memory file rather than lose the lesson.
 * That is the right call, and it deserves to work.
 *
 * Recognised as agent-written for exactly that reason. `agentMemories` exists to
 * leave a HUMAN note that wandered into the folder alone, which is a good rule
 * with one blind spot: it could not tell a wandering note from a deliberate
 * capture that had to route around a refusal, because it only knew one value. A
 * memory labelled honestly was punished with invisibility — served by nothing,
 * reachable by no session, including the one that wrote it.
 */
export const HAND_FILED_SOURCE = "hand-filed";

/** Sources that mark a file as a deliberate agent capture rather than a stray note. */
...
export function renderMemory(value: MemoryValue, links: readonly string[] = []): string {
	...
	//
	// `slugify` truncates at SLUG_MAX and the comment there says long titles are
	// "still fine as the H1" — true for reading, false for LINKING. Obsidian
	// resolves a `[[target]]` against filenames and aliases, never against an
	// H1, so a memory whose title outran the slug could not be cited by the one
	// string anyone would naturally use: its own stated claim. Memory titles are
	// claims by contract ("stated as a claim"), so they run long as a rule rather
	// than as an exception, and the note ends up unreachable by name from every
	// note that wants to reference it.
	//
	// Emitted only when the slug actually lost something, so short-titled
	// memories keep the frontmatter they have always had.
