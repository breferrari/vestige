...
const TOOLS = [
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        kind: { type: "string", enum: ["learning", "decision"], description: "Genre. 'learning' = what bit and how it was fixed. 'decision' = what was chosen and what was given up. Defaults to 'learning'; it does not affect retrieval, it tells the next reader which blank was being filled." },
        supersedes: { type: "array", items: { type: "string" }, description: "Memories this one replaces, by title or filename. They are MARKED and kept, never deleted — what was believed at the time is evidence that it changed. Use this instead of writing a second memory that disagrees with the first." },
        related: { type: "array", items: { type: "string" }, description: "Memories this one sits beside without replacing. The link is written on both files." },
      },
    },
  },
  {
