...
**Reach is what a per-project index cannot do, and it is not free.** Isolation by folder — one index per repository — retrieves a project's own memories better than anything else measured here, and cannot carry a lesson across: a memory written in one repository about a shared-library fault is absent from every other repository's index, at any *k*. Over 57 such queries, each asked by a service that did not write the memory, declared reach retrieves it into the top five **77%** of the time against a per-project index's **0%** and a shared pool's **60%**.

The cost is in the same mechanism. Carrying other projects' lessons enlarges every caller's view — 73 documents here rather than 23 — and a project's own memories are retrieved into the top five **71%** of the time rather than 98%. Against one shared pool of 183 documents, declared reach is better on both counts; against one index per project, it exchanges in-project recall for a capability that configuration does not have. How large the exchange is depends on how much of a store is genuinely shared: 31% of these memories declare org-wide reach.

Reach also decides storage — a memory reaching several projects cannot live inside one of them — so the declaration that makes it visible is the same one that puts it where all of them can read it.
